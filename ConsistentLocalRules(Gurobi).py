...
ConcurrentMIP = 1 # number of jobs to use in GUROBI
OutputFlag = 0


def report_error(s, filename='error_log.txt',EOL='\n'):
    print(s)
    ...
    def __str__(self): 
        ...
        for f in self.features:
            ...
            if ('-'+f) in self.features and np.isfinite(self.tau['-'+f]):
                ...
                #predicates.append('%s>=%.2f'%(f,self.tau[f]))
                predicates.append('%.2f<=%s'%(self.tau[f],f))
        res = ", ".join(predicates) 
        if len(res)==0:
            res='UNDEFINED MODEL'
        return(res)                             
...
class ConsistentRulesExplainer():
    ...
    def __init__(self, X, Y_global, complement_features=True):    
        ...
        assert((X.index == Y_global.index).all()), "make sure data is alligned"
    ...
    def batch_explain(self, X_batch, Y_batch, max_runtime=60, d=-1, err_message=''):
        '''
        Generate sparse explanations for a batch of observations, and max 
        support explanations with (optimal sparisity+0),(optimal sparisity+1),...,
        (optimal sparisity+d) features
        ...
        for i in range(len(X_batch)):
            print('Generating explanations for observation %d of %d'%(i+1,len(X_batch)))

            e = self.explain(X_batch.iloc[[i],:], Y_batch.iloc[[i]], objective='SPARSITY', 
                                       n_explanations=1, max_features=999999, max_runtime=max_runtime)            
            if len(e)!=1:
                 report_error(err_message+' batch_explain(): no sparse explanation found; obs. ID' + str(i))
                 continue
                 #assert(len(e)==1), 'no sparse explanation found?'            
            else:
                e['comment'] = 'MIN SPARSITY'
                e['obs. ID'] = i
                res.append(e)
                max_features = e['#Features'].iloc[0]
            
                
            for d_ in list(range(0,d+1)):
                e = self.explain(X_batch.iloc[[i],:], Y_batch.iloc[[i]], objective='SUPPORT', 
                                 n_explanations=1, max_features=max_features+d_, max_runtime=max_runtime)            
                if len(e)!=1:
                    report_error(err_message+' batch_explain(): no support explanation found; obs. ID '+str(i))
                    continue
                    #assert(len(e)==1), 'no support explanation found?'            
                else:
                    e['comment'] = 'MAX SUPPORT + %d'%d_                
                    e['obs. ID'] = i
                    res.append(e)
    ...
    def explain(self, X_e, Y_e, objective='SPARSITY', n_explanations=1, max_features=999999, max_runtime=60):
        ...
        #Y_e = np.array(Y_e) # prevent pandas bug when indexes are not sequential - perhaps better to check if indexes match when Y_e is a series
        ...
        assert(len(np.setdiff1d(Y_e, [0,1]))==0), 'currently only supports binary datasets'
        ...
        assert(type(Y_e)==pd.Series), 'Expected dataframe for X_e'
        assert((X_e.index==Y_e.index).all()), 'expecting indexes of X_e and Y_e to be alligned'
        ...
        for i in range(len(X_e)):
            #('Generating explanation %d of %d'%(i+1,len(Y_e)))
            df_i = self.__explain_local__(X_e.iloc[i], Y_e.iloc[i], objective, n_explanations, max_features, max_runtime)
    ...
    def __explain_local__(self, x_e, y_e, objective='SPARSITY', n_explanations=1, max_features=999999, max_runtime=60):
        ...
        y_e = int(y_e)          # avoid indexing issue when comparing y_e to Y_global
        ...
        if self.data_type=="BINARY":
            if objective=="SPARSITY":
                ce_list = self.__BinMinSetCover__(x_e, y_e, n_explanations, timeout=max_runtime) 
                #print(ce_list)
            else:
                ce_list = self.__BinMaxSupport__(x_e, y_e, max_features, n_explanations, timeout=max_runtime)
    ...
    def __BinMinSetCover__(self, x_e, y_e, n_explanations=1, timeout=TIME_LIMIT):
        '''
        ...
        for counter in range(n_explanations):            
            ...
            opt_model_g = Model("BinMinSetCover-%d"%self.n_models)
           
            #b = pulp.LpVariable.dicts('b', self.features, lowBound = 0, upBound = 1, cat = pulp.LpInteger)
            b = opt_model_g.addVars(self.features, name="b", vtype=GRB.BINARY)
            ...
            for s_index, s in enumerate(solutions):
                ...
                opt_model_g.addConstr(quicksum([b[f] for f in self.features if s[f]==0]) + quicksum([1-b[f] for f in self.features if s[f]==1])>= 1, 'Prohibit attaining previous solution %d'%s_index)        

                
            for f in self.features:
                if x_e[f]==0:
                    #opt_model += (b[f] == 0), "Feature %s must be 1"%f
                    opt_model_g.addConstr(b[f] ==0, "Feature %s must be 0"%f)        

   
            #opt_model += sum([b[f] for f in self.features]) # objective function
            opt_model_g.setObjective(quicksum([b[f] for f in self.features]), GRB.MINIMIZE) # minimize same labeled out of box
            ...
            for index, row in X_other.iterrows():
                ...
                opt_model_g.addConstr(quicksum([b[f] for f in self.features if row[f]==0])>= 1, "Const. obs. %s"%str(index))        
            ...
            opt_model_g.setParam('OutputFlag', OutputFlag)  
            opt_model_g.setParam('TIME_LIMIT', timeout)
            opt_model_g.setParam('ConcurrentMIP', ConcurrentMIP)
            opt_model_g.optimize()
            
            if (opt_model_g.status==GRB.INFEASIBLE):
                break
    ...
    def __BinMaxSupport__(self, x_e, y_e, max_features, n_explanations, timeout=TIME_LIMIT):
        ...
        for counter in range(n_explanations):            
            ...
            opt_model_g = Model("BinMaxSupprt-%d"%self.n_models)
            
            # decision variables
            #b = pulp.LpVariable.dicts('b', self.features, lowBound = 0, upBound = 1,cat = pulp.LpInteger)
            #r = pulp.LpVariable.dicts('r', same_indexes, lowBound = 0,upBound = 1,cat = pulp.LpInteger)
            b = opt_model_g.addVars(self.features, name="b", vtype=GRB.BINARY)
            r = opt_model_g.addVars(same_indexes, name="r", vtype=GRB.BINARY)
            ...
            opt_model_g.setObjective(quicksum([r[i] for i in same_indexes]), GRB.MAXIMIZE) # minimize same labeled out of box
            ...
            for index, row in X_other.iterrows():
                ...
                opt_model_g.addConstr(quicksum([b[f] for f in self.features if row[f]==0])>= 1, "Rule doesn't apply to obs. %s"%str(index))
            ...
            for index, row in X_same.iterrows():
                ...
                opt_model_g.addConstr(quicksum([b[f] for f in self.features if row[f]==0])<= self.p*(1-r[index]), "Const. obs. %s"%str(index))        
    
            #opt_model += (sum([b[f] for f in self.features]) <= max_features), "Max. num. of features"
            opt_model_g.addConstr(quicksum([b[f] for f in self.features])<= max_features, "Max. num. of features")  
...
class ConsistentExplanationTwoSided(BaseEstimator, ClassifierMixin):  


    def __init__(self, dict_LB={}, dict_UB={}, s=''):
        if len(s)!=0:
            self.dict_LB = {}
            self.dict_UB = {}
            for term in s.split(','):
                if term.count('<=')==2:
                    a,b,c = term.split('<=')
                    self.dict_LB[b.strip()] = float(a.strip())
                    self.dict_UB[b.strip()] = float(c.strip())
                elif term.count('<=')==1:                
                    a,b = term.split('<=')
                    if isfloat(a):
                        self.dict_LB[b.strip()] = float(a.strip())
                    else:
                        self.dict_UB[a.strip()] = float(b.strip())
                else:
                    raise Exception('shouldnt be here - unexpected inquality')
        else:            
            self.dict_LB = dict_LB.copy()
            self.dict_UB = dict_UB.copy()
        self.features =  sorted(set(self.dict_LB.keys()).union(set(self.dict_UB.keys())))
    ...
    def __str__(self):        
        predicates = []
        for f in self.features:
            if (f in self.dict_LB) and (f in self.dict_UB):
                predicates.append('%.2f<=%s<=%.2f'%(self.dict_LB[f],f,self.dict_UB[f]))
            elif (f in self.dict_LB):
                predicates.append('%.2f<=%s'%(self.dict_LB[f],f))
            elif (f in self.dict_UB):
                predicates.append('%s<=%.2f'%(f,self.dict_UB[f]))
            else:
                raise Exception('shouldnt be here')                
        res = ", ".join(predicates) 
        if len(res)==0:
    ...
    def predict(self, X, y=None):
        predictions = pd.Series(index=X.index, data=True)
        for f in self.features:
            if (f in self.dict_LB) and (f in self.dict_UB):
                predictions &=((X[f]>=self.dict_LB[f]-TOL) & (X[f]<=self.dict_UB[f]+TOL))#.astype(int)
            elif (f in self.dict_LB):
                predictions &=(X[f]>=self.dict_LB[f]-TOL)#.astype(int)
            elif (f in self.dict_UB):
                predictions &=(X[f]<=self.dict_UB[f]+TOL)#.astype(int)
            else:
                raise Exception('shouldnt be here')                
        #return (np.array(predictions.values).reshape((-1,1)))
        return (predictions)
    
    def support(self, X, Y_global):        
        '''
        Given a data matrix X, computes the number of observations that satsify
        the rule. If these labels are not consistent with y_m return -1.                
        '''       
        Y_rule = self.predict(X)
        return(sum(Y_rule))

    def measure_inconsistencies(self, X, Y_global, y_m):        
        Y_rule = self.predict(X)
        return( sum(Y_global[Y_rule] != y_m))

    def __eq__(self, other):
        if isinstance(other, ConsistentExplanationTwoSided):
            if not (self.dict_LB.keys() == other.dict_LB.keys()):
                return False
            if not (self.dict_UB.keys() == other.dict_UB.keys()):
                return False
            for f in self.dict_LB:
                if abs(self.dict_LB[f]- other.dict_LB[f])>TOL:
                    return False
            for f in self.dict_UB:
                if abs(self.dict_UB[f]- other.dict_UB[f])>TOL:
                    return False
            return True
        else:
            return False
...
if __name__ == "__main__":
    ...
    if 1: 
        ...
        X,Y_global,df = toy_binary_dataset(plot=False, print_table=True)
        ...
        cets = ConsistentExplanationTwoSided(dict_LB={'x2':0},dict_UB={'x1':1,'x2':0})
        cets.predict(X)
        cets.support(X,Y_global)
        cets.measure_inconsistencies(X, Y_global, 1)

        cets2 = ConsistentExplanationTwoSided({},{},str(cets))
        #cets2.__eq__(cets)
