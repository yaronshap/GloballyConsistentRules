...
--------
    ...
    Y_m - corresponding labels (length n)
    
        
@author: Yaron Shaposhnik (yaron.shaposhnik@gmail.com)

TODO: 
* why __ContMinSetCover__ generates the same solutions multiple times? (see example in the notebook)
* 
* 
...
class RBModel(BaseEstimator, ClassifierMixin):  
#class RBModel():  
    """
    ...
    def support(self, X, Y_m, y_m):        
        ...
        if sum(Y_m[Y_rule] != y_m)>0:
            return(-1)
        else:
            return(sum(Y_rule))

    def set_support(self, X, Y_m, y_m):
        self.properties['support'] = self.support(X, Y_m, y_m)
...
class GCRBE_Generator():
    """
    An object that generates globally consistent rule-based explanations from data.
    """
                
    def __init__(self, X, Y_m):    
        '''
        X : pd.DataFrame, data matrix
        Y_m : pd.Series, labels generated by a model
        '''
        assert(type(X)==pd.DataFrame)
        assert(type(Y_m)==pd.Series)
        assert(set(np.unique(Y_m))==set([0,1])), "Expected binary labeling"
        assert(X.shape[0]==len(Y_m)), "The dimensions of X and y do not match"
        ...
        if DEBUG and 0:
            ...
            #print(pd.concat([self.X_original,self.Y_m],axis=1))
            #print(pd.concat([self.X_expanded,self.Y_m],axis=1))
    ...
    def __str__(self): 
        '''
        Create string representation of the GCRBE
        '''       
    ...
    def __BinMinSetCover__(self, x, y_m, n_solutions=1):
        '''
        x : pd.Series
        y_m : binary prediction
        n_solutions : the maximal number of solutions to be returned

        output: list of RBModel, explanations
        '''
        start = time.time()
        assert(y_m in [0,1]), 'currently supports binary datasets'
        assert(type(x)==pd.Series)
        assert(set(self.features) == set(x.index.values))
        assert(self.data_type=='binary')

        solutions = []
        res = []
        for counter in range(n_solutions):            
            self.n_models+=1
            ...
            for f in self.features:                 
                ...
                if b[f].value()==1:
                    tau[f] = 1                 
            rbm = RBModel(tau,{'time':'%.1f'%(time.time()-start),
                               'type':'BinMinSetCover'})   
            
            rbm.set_support(self.X, self.Y_m, y_m)
    ...
    def __BinMaxSupport__(self, x, y_m, maximal_n_terms):
        ...
        x : pd.Series (compatible with original feature space)
        y_m : binary prediction
        maximal_n_terms : maximal number of terms
        
        output: RBModel, an explanation (just one)
        '''
        start = time.time()
        assert(y_m in [0,1]), 'currently supports binary datasets'
        assert(type(x)==pd.Series)
...
#            # NOT TESTED 
#            
#            for f_index, f in enumerate(self.features_original): 
#                e = initial_solution
#                assert(np.isfinite(e.tau[f_index]) and np.isfinite(e.ub[f_index]))
#                if np.isfinite(e.tau[f_index]):
#                    b[f].setInitialValue(e.tau[f_index])
#                    b['~'+f].setInitialValue(1-e.tau[f_index])
#                else:
#                    b[f].setInitialValue(0)
#                    b['~'+f].setInitialValue(0)                    
        ...
        opt_model += (sum([b[f] for f in self.features]) <= maximal_n_terms), "Max. num. of features"
        ...
        rbm = RBModel(tau,{'time':'%.1f'%(time.time()-start),
                           'type':'BinMaxSupport'})          
        rbm.set_support(self.X, self.Y_m, y_m)
        assert(rbm.predict(np.array(x).reshape((1,-1)))), 'Generated explanation does not agree with explained observation'
        return(rbm)
    ...
    def generate_explanations_for_training_data(self, obs_list, explanation_types = {}):
        ...
        res = []
        for i in obs_list:                                   
            x, y_m = self.X_original.iloc[i], self.Y_m[i]
    ...
    def __ContMinSetCover__(self, x_e, y_m, n_binary_solutions=1):
        '''
        ...
        start = time.time()  
        X_bin = (self.X >= x_e).astype(int)
        x_e_bin = (x_e>=x_e).astype(int)        
        bin_GCRBE = GCRBE_Generator(X_bin, self.Y_m)        
        
        res = []
        rbm_bin_solutions = bin_GCRBE.__BinMinSetCover__(x_e_bin, y_m, n_binary_solutions)
        for rbm_bin in rbm_bin_solutions:        
            tau_cont = pd.Series(-np.inf, index=self.features)
            for f in self.features:
                if np.isfinite(rbm_bin.tau[f]):
                    tau_cont[f]=x_e[f]    
            rbm = RBModel(tau_cont,{'time':'%.1f'%(time.time()-start),
    ...
    def __ContMaxSupport__(self, x_e, y_m, e_list=[], n_binary_solutions = 1):
        '''
        Expands a given solution to increase support while maintaining the same features. 
        
        
        x_e : observation to be explained
        y_m : corresponding label
        e : explanation to be expanded (or ContMinSetCover solution used as a starting point for expansion) 
        '''
        start = time.time()  
        if e_list==[]: 
            e_list = self.__ContMinSetCover__(x_e, y_m, n_binary_solutions) # call takes care of input validation and find the basis for expansion
        else:   
            assert(y_m in [0,1]), 'currently supports binary datasets'
            assert(type(x_e)==pd.Series)
            assert(set(self.features) == set(x_e.index.values))
            ...
            assert_cont_expanded(self.X), "__ContMinSetCover__ only works with continuously expanded datasets (call complement_continuous_dataframe)"
            for e in e_list:
                assert(e.support(self.X, self.Y_m, y_m)>-1), 'provided explanation is not consistent:'+ str(e)
        ...
        for e in e_list:
            ...
            for f in self.features:            
                if np.isfinite(tau[f]):
                    features.append(f)
                    #thresholds[f] = self.X.loc[(self.Y_m==y_m)&(self.X[f]<=x_e[f]),f].unique().tolist()                
            ...
            explored_states = {}
            self.solve_DP(tau, features, thresholds, state, y_m, explored_states)                
    ...
    def solve_DP(self, tau, features, thresholds, state, y_m, explored_states):
        '''
        ...
        y_m : binary, label of the explained observation
        ...
        #print('before:\n',explored_states)
        e_curr = RBModel(tau,{})    
        e_curr.set_support(self.X,self.Y_m,y_m)        
        ...
        for f_index, f in enumerate(features):               
            if state[f_index]==0:
                ...
                self.solve_DP(tau, features, thresholds, state_next, y_m, explored_states)                                    
...
def gen_small_dataset(plot=True, print_table=True, dtype=int):
    k = 2
    data=np.array([((i,j),i,j,0) for i in range(k) for j in range(k)])
    df = pd.DataFrame(data, columns=['i','x0','x1','y'])    
    df.index = df.iloc[:,0].values
    df = df.iloc[:,1:]
    df.loc['x'] = {'x0':1, 'x1':1, 'y':0}
    #df.loc[[(0,k_) for k_ in range(k)],'y']=1
    df.loc[[(1,1)],'y']=1    
    X, y = df[['x0','x1']], df['y']
    if dtype==float:
        X+=0.0001
...
if __name__ == "__main__":
    ...
    if 0: # test the class RBModel
        rbm = RBModel([1,2,3],[4,5,6])
        print(rbm)
        rbm = RBModel([1,2,3],[4,5,6],{'features':['a','b','c']})
        print(rbm)
        print(rbm.predict([[1,2,3],[4,5,4],[0,5,4],[4,5,10]]))
        
    if 0: 
        # experiment with toy dataset on a 3x3 grid
        X,y_m,df = gen_small_dataset(plot=True, print_table=False, dtype=int)
        le_gen = GCRBE_Generator(X, y_m)        
        le_list = le_gen.generate_explanations_for_training_data(range(len(X)), {'BinMinSetCover':None, 'BinMaxSupport':{}})
        df['LE'] = le_list
        df['n_terms'] = map(lambda c:c,le_list)
        print(df)
